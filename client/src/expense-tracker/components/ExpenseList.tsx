...
const ExpenseList = ({category, expenses, onDelete, fetchData}:ExpenseProps) => {
    ...
    const saveExpense = (id:number) => {
        ...
        if(updatingData)
        {
            axios
                ...
                .then(()=>{
                    setSavedData(expenses.map(exp => exp.id === id ? updatingData : exp));
            ...
            <tbody className="dataStyle">
                ...
                { category === '' ?
                    ...
                    <td className="dataStyle text-center">
                        ...
                        <button className="btn btn-outline-warning deleteButton" onClick={() => startUpdate(expense.id)}>Update</button>
